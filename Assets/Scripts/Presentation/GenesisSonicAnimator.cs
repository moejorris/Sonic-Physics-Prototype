...
public class GenesisSonicAnimator : MonoBehaviour
{
    public static GenesisSonicAnimator instance;
    GenesisSonicAnimations animations;
    public SonicSprite[] currentAnimation;
    ...
    void AssignReferences()
    {
        ...
        animations = playerMovement.sonicCharacter.genesisAnimations;
    ...
    void UpdateSpriteRotation()
    {
        ...
        switch(playerMovement.sonicCharacter.groundRotationStyle)
        {
            case SonicCharacter.GroundRotationStyle.Genesis:
                displayRot = SnappedAngle(zRot);
                break;

            case SonicCharacter.GroundRotationStyle.Mania:
    ...
    {
        isRunning = true;
        SonicSprite prevAnimation = null;
        ...
        while(isRunning)
        {
            for(int i = 0; i < currentAnimation.Length; i++)
            {
                ...
                while(frame < max)
                {
                    ...
                    bool hasSuperFrame = !beCircle && currentAnimation[i].superSprites.Length > 0;

                    max = isSuper && hasSuperFrame ? currentAnimation[i].superSprites.Length : currentAnimation[i].sprites.Length;

                    if(isSuper && hasSuperFrame)
                    {
                        spriteRenderer.sprite = currentAnimation[i].superSprites[frame];
                    }
                    else if(beCircle)
                    {
    ...
    {
        SonicSprite[] prevAnimation = currentAnimation;
        ...
        else if(!playerMovement.isGrounded() && animations.falling.Length > 0 && currentAnimation != animations.spring && currentAnimation != animations.diagonalSpring)
        {
    ...
    bool ShouldBrake(float absGSP)
    {
        ...
        bool finishUntilStopped = alreadyBraking && absGSP > 0 && (Mathf.Sign(InputManager.Instance.moveInputDir.x) != Mathf.Sign(playerMovement.GroundSpeed) || InputManager.Instance.moveInputDir.x == 0f) && animations.brakingStyle == GenesisSonicAnimations.BrakeStyle.Sonic1_CD;
